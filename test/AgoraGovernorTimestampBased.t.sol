...
import {ApprovalVotingModule, ProposalOption, ProposalSettings, PassingCriteria} from "src/modules/ApprovalVotingModule.sol";
...
contract AgoraGovernorTimestampBasedTest is Test {
    ...
    function setUp() public virtual {
        ...
        govToken = L2GovToken(
            address(
                new ERC1967Proxy(
                    address(new L2GovToken()),
                    abi.encodeCall(govToken.initialize, (admin, "L2 Gov Token", "gL2"))
                )
            )
        );
...
contract ExecuteTimestampBased is AgoraGovernorTimestampBasedTest {
    function testFuzz_ExecutesAProposalWhenManager(
        uint256 _proposalTargetCalldata,
        uint256 _elapsedAfterQueuing
    ) public virtual {
        _elapsedAfterQueuing = bound(_elapsedAfterQueuing, timelockDelay, type(uint208).max);
...
contract SetProposalDeadlineTimestamp is AgoraGovernorTimestampBasedTest {
    function testFuzz_SetsProposalDeadlineTimestampAsAdminOrTimelock(
